...
![Uptime Kumaアカウント作成](./images/uptimekuma_setup.png)
3.  **モニターの追加**:
    -   ダッシュボード左上の **[+ 監視の追加]** をクリック。
    ![Uptime Kuma監視追加](./images/uptimekuma_add-monitor.png)
    -   以下の通り設定する。
        -   **監視タイプ**: `TCP Port`
        -   **分かりやすい名前**: `otetekart`
...
2.  **ゲームプレイ**:
    -   メインメニューで「VS Race」を選択する。
    ![otetekartメニュー選択](./images/otetekart_vs-race.png)
    -   キャラクターを選択する。
    ![otetekartキャラクター選択](./images/otetekart_character-select.png)
    -   ルーム（例: Room 0）に入る。
    ![otetekartルーム選択](./images/otetekart_room-select.png)
    -   レースが始まり、キャラクターを前に移動させると、もう一方の画面でリアルタイムに相手のキャラクターの位置が変わる。
    ![otetekart同期時1](./images/otetekart_multiplay-sync1.png)
    ![otetekart同期時2](./images/otetekart_multiplay-sync2.png)
